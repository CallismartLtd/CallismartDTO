 ...
 *
 * A flexible, extendable foundation for DTOs that supports dynamic properties,
 * array access, iteration, and JSON serialization. Designed to be included
 * in standalone packages without external dependencies.
 *
...
class DTO implements \IteratorAggregate, \Countable, \ArrayAccess, \JsonSerializable {
    ...
    */
     ...
     * InvalidArgumentException. Return an empty array (default) to
     * allow any key.
     *
     * @return string[]
     */
    ...
    /**
     * Return keys that should be masked in debugging and serialization.
     *
     * Default implementation returns empty array. Override to protect
     * sensitive values like passwords, API keys, etc.
     *
     * @return string[]
     */
    protected function sensitive_keys(): array {
     ...
     *
     * Override in subclasses to enforce types, trim strings, validate
     * values, etc. The default implementation returns the value unchanged.
     *
     * @param string $key   The property name being set.
     * @param mixed  $value The raw value.
     * @return mixed The (optionally transformed) value to store.
     *
     * @throws InvalidArgumentException For invalid values that fail validation.
     */
    protected function cast( string $key, mixed $value ): mixed {
        return $value;
    }
    ...
    | CORE INTERNAL WRITE
    |
    | All writes funnel through here so that allowed_keys()
    | and cast() are always respected.
    |---------------------------------------------------------
    */
     ...
     * through here so that allowed_keys() and cast() are always honored.
     *
     * @param string $key
     * @param mixed  $value
     * @return void
    ...
    */
    ...
    | are always honored, even through magic access.
    |---------------------------------------------------------
    */
